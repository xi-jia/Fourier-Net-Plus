...
for stct in outstruct:
    tar_idx = []
    with open(file_dir+'L2ss_2_Chan_16_Smth_{}.0_LR_0.0001_Val_Test.csv'.format(file_index), "r") as f:
...
print(exp_data.std())
my_list = []
with open(file_dir+'L2ss_2_Chan_16_Smth_{}.0_LR_0.0001_Val_Test.csv'.format(file_index), newline='') as f:
    reader = csv.reader(f)
    my_list = [row[-1] for row in reader]
my_list = my_list[2:]
my_list = np.array([float(i) for i in my_list])*100
print('jec_det: {} +- {}'.format(my_list.mean(), my_list.std()))
